            ...
            <div className={`max-w-[80vw] sm:max-w-[65vw] md:max-w-[500px] transition-opacity duration-1000 ${loaded ? "opacity-100" : "opacity-0"}`}>
                <div className="flex justify-between items-start">
                    ...
                    <Image height={110} width={110} priority
                     className={`lg:hidden -translate-y-14 duration-1000 ${loaded ? "translate-x-0 opacity-100" : "translate-x-[50%] opacity-0"}`}  src="/images/scroll.png" alt="" />
                </div>
...
{/*main images with animation */}
            ...
            <div className="relative mt-24 max-w-[80vw]">
                ...
                <Image height={162} width={135} priority
                 className={`absolute bottom-0 left-0 sm:left-8 sm:w-[38%] xl:w-[40%] transition-transform duration-1000 ${loaded ? "translate-x-0 opacity-100" : "translate-x-[-100%] opacity-0"}`} src="/images/lawyer.png" alt="" />
                <Image height={253} width={250} priority
                 className={`absolute bottom-0 right-[-25px] sm:right-0  sm:w-[70%] xl:w-[75%] transition-transform duration-1000 ${loaded ? "translate-x-0 opacity-100" : "translate-x-[20%] opacity-0"}`} src="/images/kids.png" alt="" />
                <Image height={104} width={156} priority
                 className={`hidden lg:block absolute xl:top-[-130px] right-0 xl:w-[25%] lg:w-[20%] lg:top-[-70px] transition-transform duration-1000 ${loaded ? "translate-y-0 opacity-100" : "translate-y-[-100%] opacity-0"}`} src="/images/scroll.png" alt="" />
                <Image height={113} width={115} priority
                 className={`hidden xl:block absolute bottom-5 left-[-170px] transition-transform duration-1000 ${loaded ? "translate-x-0 opacity-100" : "translate-x-[-100%] opacity-0"}`} src="/images/bag.png" alt="" />
            </div>
        </div>
    );
};
